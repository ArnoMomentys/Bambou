...
var watcher = chokidar.watch(config.upload_folder_guests, { ignored: /[\/\\]\./, persistent: true });
...
connection.connect(function(err) {
    ...
    console.log('Connected for importing guests as id ' + connection.threadId);
});
      ...
      .on('end', function() {
        ...
        fs.unlink(ress, function (err) {
            ...
            var t = my_table                    // ex : g1e100t1404710305
                ...
                ,status = arr[0].charAt(0)      // 1: uid de l'utilisateur qui a importé le fichier
                ,uid = arr[0].substr(1)         // e: event
                ,eid = arr[1].substr(1)         // 100: id de l'evenement
                ,time = arr[2].substr(1)       // t: time // 1404710305: timestamp de l'import
                ,ret;                           // process return

                var guests = require('./g');
                console.log('Processing guests list...');
                guests.process(my_table, uid, eid);
