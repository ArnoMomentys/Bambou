...
<h3 class="pull-left disblo widful page-subheading padtop20 marbotnon">
    <?php echo $event_guest_add[$l]; ?><br>
    <small class="fonweilig"><?php echo $choose_in_list_user_to_add[$l]; ?></small><br>
    <small class="required"><?php echo $guest_not_listed[$l]; ?></small>
    <?php if ($isold == false): ?>
        <a href="<?php echo '/event/'.$event['eid'].'/add/new/guest'; ?>" class="pull-right texdecnon postop-10 posrel" title="<?php echo $event_guest_add_new[$l]; ?>">
            <small class="hidden-xs hidden-sm"><?php echo $event_guest_add_new[$l]; ?></small>
                        ...
                        <th class="<?php echo $value; ?>">
                            <div class="th-wrapper">
                                ...
                                <div class="sorterwrap" with="<?php $_value=$value=='nomcomplet'?'nom':$value;$k=$$_value; ?>">
                                    <a href="<?php echo $BASE.'/event/'.$event['eid'].'/add/guest/'.$_value.($_value==$filter && strlen($filtervalue)>0 ? '/'.$filtervalue:'').'/order/asc'; ?>"
                                    ...
                                    <a href="<?php echo $BASE.'/event/'.$event['eid'].'/add/guest/'.$_value.($_value==$filter && strlen($filtervalue)>0 ? '/'.$filtervalue:'').'/order/desc'; ?>"
                    ...
                    <?php if (trim($liste['nom'])!=''): ?>
                        <tr>
                                ...
                                <?php if (in_array($key, ['nomcomplet','societe','fonction','ville','adresse'])): ?>
                                    <td class="<?php echo $key; ?>">
                                        <?php if ($key=='nomcomplet'): ?>
                                            ...
                                            <?php else: ?>
                                                <span class="<?php echo ($key=='ville'?'textraupp ':''); ?>disinlblo widful"><?php echo $value; ?></span>
                            ...
                            <td class="actions texalirig">
                                <span class="btn btn-link curpoi has-tip" data-toggle="modal" data-target="#myModal-addguest-<?php echo $liste['uid']; ?>" title="<?php echo $add[$l]; ?>">
                                    <i class="glyphicon glyphicon-plus glyphicon-black"></i>
                                </span>
                                <div class="modal fade bs-example-modal-sm" id="myModal-addguest-<?php echo $liste['uid']; ?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel-<?php echo $liste['uid']; ?>" aria-hidden="true">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <div class="modal-header bornon texalilef">
                                                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                                                <h4 class="modal-title" id="myModalLabel-<?php echo $liste['uid']; ?>">
                                                    <i class="glyphicon glyphicon-info-sign"></i> <?php echo ucfirst($add[$l]); ?> <b class="fonstyita"><?php echo $liste['nomcomplet']; ?></b> <?php echo $to_event_guests[$l]; ?>
                                                </h4>
                                            </div>
                                            <form action="<?php echo $BASE.'/event/'.$event['eid'].'/add/guest'; ?>" method="post" class="form-delete" role="form" id="form-addguest-<?php echo $liste['uid']; ?>">
                                                <div class="modal-footer bornon padtopnon martopnon">
                                                    ...
                                                    <input type="hidden" name="guestID" value="<?php echo $liste['uid']; ?>">
                                                    <input type="hidden" name="hostID" value="<?php echo $SESSION['uid']; ?>">
                                                    <button type="button" class="btn btn-default" data-dismiss="modal">
                                                        <?php echo $cancel[$l]; ?>
                                                    </button>
                                                    <button class="btn btn-info" type="submit">
                                                        <?php echo $event_guest_addto_confirm[$l]; ?>
                                                    </button>
                                                </div>
                                            </form>
                                        </div>
